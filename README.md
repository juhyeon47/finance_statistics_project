...
- **순이익률** : 당기순이익/매출액 * 100
  ...
  - 한계 : 다른 산업간 수익성 비교 X, 자본구조가 다른 기업간 비교 X, 유형자산 비중 높은 회사는 높은 감가상각비로 순이익률 낮아짐, 높은 순이익률 기록했더라도 현금창출로 이어지지 않는 경우 의미 X(재무제표를 기록하는 회계는 **발생주**의에 근거하기 때문)
  - **LX인터내셔널의 순이익률이 더 높음** => 수익성 측면에서 우위에 있다.

- **ROE** : 당기순이익/자기자본 * 100
  - 값이 높을 수록 적은 자본으로 많은 이익을 얻고있다는 의미 => 수익성 좋은
